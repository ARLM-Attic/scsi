...
namespace FileSystems.Udf
{
	public class UdfMaster : DiscMaster
	{
		...
		private void CreateVolumeDescriptorSequence(UdfRevision udfVersion, AnchorVolumeDescriptorPointer anchor, out PrimaryVolumeDescriptor pvd, out PartitionDescriptor pd, out LogicalVolumeDescriptor lvd, out LogicalVolumeIntegrityDescriptor integrityDescriptor, out TaggedDescriptor[] descriptors)
		{
			...
			integrityDescriptor = new LogicalVolumeIntegrityDescriptor()
			{
				...
				MaximumUdfWriteRevision = UdfRevision.Version0150,
			};
		...
		public override AllocatedStream CreateStream()
		{
			/*
			 * Order:
			 * 1. Create and allocate the Volume Recognition Sequence (VRS)
			 * 2. Create and allocate the Volume Descriptor Sequence (VDS)
			 * 3. For each directory (start at root)
			 *    a. Add all File Identifier Descriptors (FIDs)
			 *    b. Add all files' File Entries (FE ICBs)
			 *    c. Add all folders' File Entries (FE ICBs)
			 *   --> NOTE: You CANNOT perform (b) and (c) without first going through all files... how do you fix this?
			 * 4. Perform modifications on file system as needed
			//*/


			this.DuplicateFilesFound = new Dictionary<IStreamSource, List<IStreamSource>>(new StreamEqualityComparer(this.UnbufferedReads));
			...
			this.CreateVolumeDescriptorSequence(UdfRevision.Version0102, anchor, out pvd, out pd, out lvd, out integrityDescriptor, out descriptors);

			List<KeyValuePair<Range<long>, IStreamSource>> volumeAllocation;

#if ISO9660
			var isoPVD = new Iso9660.PrimaryVolumeDescriptor()
			{
				ApplicationIdentifier = pvd.ApplicationIdentifier.IdentifierToString(),
				VolumeIdentifier = pvd.VolumeIdentifier,
				VolumeSetIdentifier = pvd.VolumeSetIdentifier,
				LogicalBlockSize = checked((ushort)lvd.LogicalBlockSize),
			};

			var isoSVD = new Iso9660.SupplementaryVolumeDescriptor()
			{
				AbstractFileIdentifier = isoPVD.AbstractFileIdentifier,
				ApplicationIdentifier = isoPVD.ApplicationIdentifier,
				//ApplicationUse = isoPVD.ApplicationUse,
				BibliographicFileIdentifier = isoPVD.BibliographicFileIdentifier,
				BigEndianPathTableLocation = isoPVD.BigEndianPathTableLocation,
				CopyrightFileIdentifier = isoPVD.CopyrightFileIdentifier,
				DataPreparerIdentifier = isoPVD.DataPreparerIdentifier,
				FileStructureVersion = isoPVD.FileStructureVersion,
				LittleEndianPathTableLocation = isoPVD.LittleEndianPathTableLocation,
				LogicalBlockSize = isoPVD.LogicalBlockSize,
				OptionalBigEndianPathTableLocation = isoPVD.OptionalBigEndianPathTableLocation,
				OptionalLittleEndianPathTableLocation = isoPVD.OptionalLittleEndianPathTableLocation,
				PathTableSize = isoPVD.PathTableSize,
				PublisherIdentifier = isoPVD.PublisherIdentifier,
				//RootDirectoryRecordData = isoPVD.RootDirectoryRecordData,
				StandardIdentifier = isoPVD.StandardIdentifier,
				SystemIdentifier = isoPVD.SystemIdentifier,
				Type = isoPVD.Type,
				Version = isoPVD.Version,
				VolumeCreationTime = isoPVD.VolumeCreationTime,
				VolumeEffectiveTime = isoPVD.VolumeEffectiveTime,
				VolumeExpirationTime = isoPVD.VolumeExpirationTime,
				VolumeIdentifier = isoPVD.VolumeIdentifier,
				VolumeModificationTime = isoPVD.VolumeModificationTime,
				VolumeSequenceNumber = isoPVD.VolumeSequenceNumber,
				VolumeSetIdentifier = isoPVD.VolumeSetIdentifier,
				VolumeSetSize = isoPVD.VolumeSetSize,
				VolumeSpaceSize = isoPVD.VolumeSpaceSize,
			};
			isoPVD.ApplicationUse.CopyTo(isoSVD.ApplicationUse, 0);
			isoPVD.RootDirectoryRecordData.CopyTo(isoSVD.RootDirectoryRecordData, 0);
#endif
			...
			var sameFileCache = this.EncodeDuplicatesOnce ? new UniqueStreamCollection<FileEntryInformationControlBlockDescriptorBase>(this.UnbufferedReads) : null;
				...
				while (stack.Count > 0)
				{
					...
					for (int i = 0; i < fileInfos.Count; i++)
					{
						...
						if (info.FileEntry.TagLocation <= 0)
						{
							...
							nextFreeByte += lvd.LogicalBlockSize;
							if (info.FileEntry.FileType == IcbFileType.Directory) { var dir = (IDirectoryInfo)info.File; }
