...
namespace FileSystems
{
	public abstract class DiscMaster : IStreamSource
	{
		...
		public DiscMaster(int sectorSize) { this.SectorSize = sectorSize; }
		public VirtualDirectoryInfo Root { get { return this._Root; } set { this._Root = value; } }
		protected virtual void OnBuildProgress(ProgressEventArgs e) { e.Cancel = false; if (this.BuildProgress != null) { this.BuildProgress(this, e); } }
		...
		public abstract AllocatedStream CreateStream();
	...
	{
		private byte[] bufferA = new byte[1024 * 1024 * 4];
		private byte[] bufferB = new byte[1024 * 1024 * 4];
		private bool bypassBuffers;

		public StreamEqualityComparer(bool bypassBuffers) : base() { this.bypassBuffers = bypassBuffers; }
		...
		public int Compare(IStreamSource x, IStreamSource y)
		{
			...
			else
			{
				...
				{
					compare = 0;
					using (var a = x.Open(FileMode.Open, FileAccess.Read, FileShare.ReadWrite, FileOptions.Asynchronous | FileOptions.SequentialScan, this.bypassBuffers))
					{
						using (var b = y.Open(FileMode.Open, FileAccess.Read, FileShare.ReadWrite, FileOptions.Asynchronous | FileOptions.SequentialScan, this.bypassBuffers))
						{
							while (compare == 0 && a.Position < xLen && b.Position < xLen)
							{
								var asyncA = a.BeginRead(this.bufferA, 0, this.bufferA.Length, null, null);
								var asyncB = b.BeginRead(this.bufferB, 0, this.bufferB.Length, null, null);
								var rA = a.EndRead(asyncA);
								var rB = b.EndRead(asyncB);
								if (rA == rB)
								{
	...
	{
		private StreamEqualityComparer comparer;

		private IDictionary<long, SortedList<IStreamSource, TValue>> uniqueFiles = new Dictionary<long, SortedList<IStreamSource, TValue>>();

		public UniqueStreamCollection(bool bypassBuffers) { this.comparer = new StreamEqualityComparer(bypassBuffers); }
