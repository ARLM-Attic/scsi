...
namespace BurnApp
{
	public partial class BackgroundBurner : BackgroundWorker
	{
					...
					if ((DialogResult)this.Owner.Invoke((Converter<IWin32Window, DialogResult>)(window => MessageBox.Show(window, "Would you like to erase the disc? Any data on it will become inaccessible.", "Erase Disc", MessageBoxButtons.OKCancel, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1)), this.Owner) == DialogResult.OK)
					{
						...
						try
						{
							...
							for (uint i = 0; i < 16; i += blockingFactor)
							{
								...
								device.Write(false, false, i, Math.Min(blockingFactor, 16 - i), new byte[device.BlockSize * Math.Min(blockingFactor, 16 - i)], 0);
							...
							for (uint i = 16; i < 256; i += blockingFactor)
							{
								...
								device.Write(false, false, i, Math.Min(blockingFactor, 256 - i), new byte[device.BlockSize * Math.Min(blockingFactor, 256 - i)], 0);
								current += blockingFactor;
							}
							...
							if (!this.CancellationPending)
							{
								...
								device.Write(false, false, 256, 1, new byte[device.BlockSize], 0);
								current += 1;
							}
							else { e.Cancel = true; }
							...
							if (!this.CancellationPending)
							{
								...
								device.Write(false, false, blockCount - 1 - 256, 1, new byte[device.BlockSize], 0);
								current += 1;
							}
							else { e.Cancel = true; }
							...
							if (!this.CancellationPending)
							{
								...
								device.Write(false, false, blockCount - 1, 1, new byte[device.BlockSize], 0);
								current += 1;
							}
							else { e.Cancel = true; }
						}
						...
						{
							this.ReportProgress(new ProgressInfo(BurnStage.Blanking, MasterStage.None, current, total, "Flushing drive buffer", null, string.Empty), true);
				...
				try
				{
					...
					if (info.Type == BurnType.Burn)
					{
						...
						using (var target = info.TargetDevice == null
							? info.TargetImage.Open(FileMode.Create, FileAccess.ReadWrite, FileShare.Read, FileOptions.SequentialScan, info.UnbufferedWrites)
							: info.TargetDevice.OpenTrack(trackNumber))
						{
							...
							source = info.Source.Open(FileMode.Open, FileAccess.Read, FileShare.Read, FileOptions.SequentialScan, info.UnbufferedReads);
							...
							if (!this.CancellationPending)
							{
								using (source)
								{
									...
									if (info.TargetDevice != null)
									{
										var wp = info.TargetDevice.GetWriteParameters(new ModeSense06Command(PageControl.CurrentValues));
										if (wp.WriteType == WriteType.SessionAtOnce)
										{
											...
											this.SendCueSheet(info.TargetDevice, trackNumber, wp, (source.Length - trackStartOffset));
										}
									}
									...
									if (!this.CancellationPending)
									{
										...
										this.Write(source, info.TargetDevice, target, trackStartOffset, blockSize, info.BufferSize, info.UnbufferedReads);
									}

									if (!this.CancellationPending && info.ClearArchiveBits && master != null)
									{
										this.ClearArchiveBits(master.Root);
									}
								}
							}
							else { e.Cancel = true; }
						...
						if (info.TargetDevice != null)
						{
							...
							{
								//Close track
								info.TargetDevice.CloseTrackOrSession(new CloseSessionTrackCommand(true, TrackSessionCloseFunction.CloseLogicalTrack, trackNumber));
								int prevTick = Environment.TickCount;
								//Get the progress
								SenseData sense;
								while ((sense = info.TargetDevice.RequestSense()).SenseKey == SenseKey.NotReady && sense.AdditionalSenseCode == AdditionalSenseCode.LogicalUnitNotReady && sense.AdditionalSenseCodeQualifier == (AdditionalSenseCodeQualifier)7)
								{
									...
									if (tick - prevTick >= Program.SLOW_UPDATE_PAUSE_MILLIS)
									{
										this.ReportProgress(new ProgressInfo(BurnStage.Closing, MasterStage.None, sense.SenseKeySpecific.ProgressIndication.ProgressIndication, ProgressIndicationBytes.ProgressIndicationDenominator, "Closing track", "(cannot be canceled)", string.Empty), false);
										prevTick = tick;
									}
									else { Thread.Sleep(Program.SLOW_UPDATE_PAUSE_MILLIS); }
								}
								this.ReportProgress(new ProgressInfo(BurnStage.Closing, MasterStage.None, ProgressIndicationBytes.ProgressIndicationDenominator, ProgressIndicationBytes.ProgressIndicationDenominator, "Track closed.", null, string.Empty), false);
							}
		...
		private void ClearArchiveBits(VirtualDirectoryInfo virtualDirectoryInfo)
		{
			var infos = new List<IFileSystemInfo>();
			var q = new Queue<IDirectoryInfo>();
			q.Enqueue(virtualDirectoryInfo);
			while (q.Count > 0)
			{
				var next = q.Dequeue();
				foreach (var item in next.GetFileSystemInfos("*", SearchOption.TopDirectoryOnly))
				{
					var asDir = item as IDirectoryInfo;
					infos.Add(item);
					if (asDir != null) { q.Enqueue(asDir); }
				}
			}

			var pi = new ProgressInfo(BurnStage.PerformingFinalSteps, MasterStage.Writing, 0, infos.Count, "Clearing archive bits...", null, "file");
			pi.Total = infos.Count;
			foreach (var item in infos)
			{
				IFileSystemInfo target;
				VirtualFileSystemInfo virtItem = item as VirtualFileSystemInfo;
				if (virtItem == null) { target = item; }
				else { target = virtItem.LinkedTarget; }
				var newAttr = target.Attributes & ~FileAttributes.Archive;
				if (newAttr == 0) { newAttr = FileAttributes.Normal; }
				target.Attributes = newAttr;
				pi.Completed++;
				pi.State = target;
				this.ReportProgress((int)(100 * (double)pi.Completed / (double)pi.Total), pi);
			}
			this.ReportProgress(100, pi);
		}
		...
		private void SendCueSheet(MultimediaDevice multimediaDevice, int trackNumber, WriteParametersPage writeParameters, long length)
		{
			...
			TRY_WRITE:
				try { multimediaDevice.Write(false, false, (uint)Msf.Zero, (uint)bytes.Length / sectorSize, bytes, 0); }
				catch (Exception ex)
				{
		...
		private void Write(Stream source, MultimediaDevice device, Stream target, long trackOffset, int blockSize, int bufferSize, bool unbufferedReads)
		{
			...
			var asAllocatedStream = source as AllocatedStream;
			if (asAllocatedStream != null) { asAllocatedStream.BypassBuffers = unbufferedReads; }
			...
			using (var ring = new RingBuffer(bufferSize, Program.POLL_MILLIS, Program.POLL_SPIN))
			{
				...
				try
				{
					while (source.Position < source.Length)
					{
						...
						pi.State = asAllocatedStream != null ? asAllocatedStream.CurrentSource : (object)string.Empty;
				...
				finally
				{
					async.Stop();
					writeThread.Join();
					this.ReportProgress(new ProgressInfo(BurnStage.FlushingDrive, MasterStage.None, 0, 1, "Flushing and writing lead-out", null, string.Empty), false);
					target.Flush();
					this.ReportProgress(new ProgressInfo(BurnStage.FlushingDrive, MasterStage.None, 1, 1, "Flushing and writing lead-out", null, string.Empty), false);
				}
			}
		}
	...
	internal class DoWorkInfo
	{
		...
		public bool UnbufferedReads;
		public bool UnbufferedWrites;
		public bool ClearArchiveBits;
	}

	internal enum BurnStage { None, Locking, Dismounting, FlushingBuffer, FlushingDrive, SettingParameters, Blanking, Reserving, LeadIn, Burning, LeadOut, Closing, Unlocking, PerformingFinalSteps }
