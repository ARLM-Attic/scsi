...
namespace BurnApp
{
	public partial class CapacityBar : Control
	{
		...
		internal static string ToSizeString(long size) { return size < 1 << 10 ? string.Format("{0:N0} bytes", size) : (size < 1 << 20 ? string.Format("{0:N0} KiB", size / (double)1024) : (size < 1 << 30 ? string.Format("{0:N0} MiB", size / (double)(1024 * 1024)) : string.Format("{0:N1} GiB", size / (double)(1024 * 1024 * 1024)))); }
		...
		protected override void OnPaint(PaintEventArgs pe)
		{
			...
			using (var font = new Font(this.Font.FontFamily, this.Font.Size))
			{
				...
				var maxLabelSize = TextRenderer.MeasureText(pe.Graphics, ToSizeString(max), font, this.ClientSize, flags);
