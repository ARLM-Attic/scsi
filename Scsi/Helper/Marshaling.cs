...
namespace Helper
{
	...
	public static class Marshaler
	{
		...
		//Note: we must support UNALIGNED data here (some of my projects need this)... but I don't.
		[DebuggerHidden, DebuggerNonUserCode]
		[CLSCompliant(false)]
		public static unsafe void StringToPtrUni(string @string, char* ptr) { for (int i = 0; i < @string.Length; i++) { ptr[i] = @string[i]; } }
		...
		[DebuggerHidden, DebuggerNonUserCode]
		[CLSCompliant(false)]
		public static unsafe void StringToPtrAnsi(string @string, byte* ptr) { for (int i = 0; i < @string.Length; i++) { ptr[i] = checked((byte)@string[i]); } }

		//Note: we must support UNALIGNED data here (some of my projects need this)... but I don't.
		[DebuggerHidden, DebuggerNonUserCode]
		public static unsafe void StringToPtrAnsi(string @string, IntPtr ptr) { StringToPtrAnsi(@string, (byte*)ptr); }
	...
	{
		/// <summary>Converts the given value from/to big-endian (MSB-first) format, to/from the platform format.</summary>
		public static T BigEndian<T>(T value) { return BitConverter.IsLittleEndian ? ReverseBytes(value) : value; }

		/// <summary>Converts the given value from/to little-endian (LSB-first) format, to/from the platform format.</summary>
