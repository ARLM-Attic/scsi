...
namespace Helper.IO
{
	...
	public class Win32FileStream : Stream //Can be a directory as well!
	{
		...
		public Win32FileAttributes AttributesWin32 { get { return UnsafeNativeMethods.NtQueryBasicInformationFile(this.SafeFileHandle).FileAttributes; } set { var bi = UnsafeNativeMethods.NtQueryBasicInformationFile(this.SafeFileHandle); bi.FileAttributes = value; UnsafeNativeMethods.NtSetBasicInformationFile(this.SafeFileHandle, bi); } }
		...
		public DateTime ChangeTime { get { return DateTime.FromFileTime(UnsafeNativeMethods.NtQueryBasicInformationFile(this.SafeFileHandle).ChangeTime); } set { var bi = UnsafeNativeMethods.NtQueryBasicInformationFile(this.SafeFileHandle); bi.ChangeTime = value.ToFileTime(); UnsafeNativeMethods.NtSetBasicInformationFile(this.SafeFileHandle, bi); } }
		public DateTime CreationTime { get { return DateTime.FromFileTime(UnsafeNativeMethods.NtQueryBasicInformationFile(this.SafeFileHandle).CreationTime); } set { var bi = UnsafeNativeMethods.NtQueryBasicInformationFile(this.SafeFileHandle); bi.CreationTime = value.ToFileTime(); UnsafeNativeMethods.NtSetBasicInformationFile(this.SafeFileHandle, bi); } }
		...
		public DateTime LastAccessTime { get { return DateTime.FromFileTime(UnsafeNativeMethods.NtQueryBasicInformationFile(this.SafeFileHandle).LastAccessTime); } set { var bi = UnsafeNativeMethods.NtQueryBasicInformationFile(this.SafeFileHandle); bi.LastAccessTime = value.ToFileTime(); UnsafeNativeMethods.NtSetBasicInformationFile(this.SafeFileHandle, bi); } }
		public DateTime LastWriteTime { get { return DateTime.FromFileTime(UnsafeNativeMethods.NtQueryBasicInformationFile(this.SafeFileHandle).LastWriteTime); } set { var bi = UnsafeNativeMethods.NtQueryBasicInformationFile(this.SafeFileHandle); bi.LastWriteTime = value.ToFileTime(); UnsafeNativeMethods.NtSetBasicInformationFile(this.SafeFileHandle, bi); } }
		...
		public WinNTFileOptions Mode { get { return UnsafeNativeMethods.NtQueryModeInformationFile(this.SafeFileHandle); } }
		...
		//The position is updated inside here; do NOT re-update it!
		...
		private int EndProcessing(IAsyncResult asyncResult, bool write)
		{
			...
			if (result != null)
			{
				...
				try
				{
					...
					using (result.AsyncWaitHandle)
					{
						if (!asyncResult.CompletedSynchronously)
						{
							unsafe
							{
								UnsafeNativeMethods.GetOverlappedResult(result.SafeFileHandle, result.Overlapped, out bytesTransferred, true); /*result.AsyncWaitHandle.WaitOne();*/
							}
						}
						else { bytesTransferred = result.Length; }
					}
				}
				...
				return bytesTransferred; //BUG: I changed this later but didn't give it much thought; is it buggy?
			}
		...
		public void SetBasicInfo(Win32FileAttributes attributes, long changeTime, long creationTime, long lastAccessTime, long lastWriteTime) { UnsafeNativeMethods.NtSetBasicInformationFile(this.SafeFileHandle, new UnsafeNativeMethods.FileBasicInformation() { FileAttributes = attributes, CreationTime = creationTime, ChangeTime = changeTime, LastWriteTime = lastWriteTime, LastAccessTime = lastAccessTime }); }
		...
		public static bool NtFileExists(SafeFileHandle root, string kernelFileName) { return UnsafeNativeMethods.NtFileExists(root, kernelFileName); }
	...
	internal static partial class UnsafeNativeMethods //Methods with implementation only
	{
		...
		public static bool NtFileExists(SafeFileHandle root, string kernelFileName)
		{
			bool success = false;
			try
			{
				...
				unsafe
				{
					fixed (char* pName = kernelFileName)
					{
						...
						var oa = new ObjectAttributes(root != null ? root.DangerousGetHandle() : IntPtr.Zero, &name, WinNTObjectAttributes.CaseInsensitive);
						IOStatusBlock iosb;
						SafeFileHandle handle;
						int ntStatus = NtOpenFile(out handle, Win32FileAccess.ReadAttributes, ref oa, out iosb, FileShare.ReadWrite | FileShare.Delete, WinNTFileOptions.None);
						if (ntStatus >= 0) { handle.Close(); }
						return ntStatus != unchecked((int)0xC0000034) && ntStatus != unchecked((int)0xC000003A) && ntStatus != unchecked((int)0xC000000F);
					}
				}
			}
