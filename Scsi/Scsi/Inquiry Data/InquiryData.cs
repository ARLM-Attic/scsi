...
namespace Scsi
{
	...
	public abstract class InquiryData : IMarshalable
	{
		...
		private const byte PERIPHERAL_DEVICE_TYPE_MASK = 0x1F;
		[DebuggerBrowsable(DebuggerBrowsableState.Never)]
		private const byte PERIPHERAL_QUALIFIER_MASK = 0xE0;
		[DebuggerBrowsable(DebuggerBrowsableState.Never)]
		private static readonly IntPtr BYTE0_OFFSET = Marshal.OffsetOf(typeof(InquiryData), "byte0");
		...
		{
			get { return (PeripheralQualifier)Bits.GetValueMask(this.byte0, 5, PERIPHERAL_QUALIFIER_MASK); }
			set { this.byte0 = Bits.PutValueMask(this.byte0, (byte)value, 5, PERIPHERAL_QUALIFIER_MASK); }
		}
		public PeripheralDeviceType PeripheralDeviceType
		{
			get { return (PeripheralDeviceType)Bits.GetValueMask(this.byte0, 0, PERIPHERAL_DEVICE_TYPE_MASK); }
			set { this.byte0 = Bits.PutValueMask(this.byte0, (byte)value, 0, PERIPHERAL_DEVICE_TYPE_MASK); }
		}
		...
		protected virtual void MarshalFrom(BufferWithSize buffer) { this.byte0 = buffer.Read<byte>(BYTE0_OFFSET); }
		protected virtual void MarshalTo(BufferWithSize buffer) { buffer.Write(this.byte0, BYTE0_OFFSET); }
		[DebuggerBrowsable(DebuggerBrowsableState.Never)]
		protected virtual int MarshaledSize { get { return Marshaler.DefaultSizeOf<InquiryData>(); } }
	}
