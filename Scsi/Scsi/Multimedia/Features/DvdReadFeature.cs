		...
		public override FeatureSupportKind GetSupport(ScsiCommand command)
		{
			return Array.BinarySearch<ScsiCommandCode>(__SupportedOperations, command.OpCode) >= 0
				&& (command.OpCode != ScsiCommandCode.ReadDiscStructure
					|| (((ReadDiscStructureCommand)command).Format == 0 || ((ReadDiscStructureCommand)command).Format == 1 || ((ReadDiscStructureCommand)command).Format == 3 || ((ReadDiscStructureCommand)command).Format == 4))
				&& (command.OpCode != ScsiCommandCode.ReadTocPmaAtip || (((ReadTocPmaAtipCommand)command).Format == ReadTocPmaAtipDataFormat.FormattedTableOfContents || ((ReadTocPmaAtipCommand)command).Format == ReadTocPmaAtipDataFormat.MultisessionInformation))
