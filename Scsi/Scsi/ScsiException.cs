...
namespace Scsi
{
	...
	{
		[NonSerialized]
		private bool msgProvided = false;
		...
		public ScsiException(SenseData senseData, string message, Exception innerException)
			: base(message, innerException) { this.SenseData = senseData; this.msgProvided = message != null; }
		...
		public override string Message
		{
			get
			{
				if (!this.msgProvided)
				{
					var senseData = this.SenseData;
					var optionals = new System.Collections.Generic.List<string>();
					if (senseData.IncorrectLengthIndicator) { optionals.Add("Incorrect Length"); }
					if (senseData.EndOfMedium) { optionals.Add("End of Medium"); }
					if (senseData.FileMark) { optionals.Add("File Mark"); }
					object senseKeySpecific = senseData.GetSenseKeySpecificBytes();
					if (senseKeySpecific != null) { optionals.Add(senseKeySpecific.ToString()); }
					return string.Format("{0}: {1}" + System.Environment.NewLine + "{2}{3}",
						...
						GetMessage(senseData.SenseKey, senseData.AdditionalSenseCodeAndQualifier),
							optionals.Count > 0 ? string.Format(" ({0})", string.Join(", ", optionals.ToArray())) : string.Empty);
				}
		...
		public static Exception CreateException(SenseData senseData, bool forceScsiException)
		{
			...
			if (!forceScsiException)
			{
				...
				{
					case SenseKey.RecoveredError:
						ex = new IOException("The command completed successfully, with some recovery action performed by the device server." + Environment.NewLine + Environment.NewLine + "Details: " + GetMessage(senseData.SenseKey, senseData.AdditionalSenseCodeAndQualifier), ex);
						break;
					case SenseKey.NotReady:
						ex = new IOException("The logical unit is not accessible. Operator intervention may be required to correct this condition." + Environment.NewLine + Environment.NewLine + "Details: " + GetMessage(senseData.SenseKey, senseData.AdditionalSenseCodeAndQualifier), ex);
						break;
					case SenseKey.MediumError:
						ex = new IOException("the command terminated with a non-recovered error condition that may have been caused by a flaw in the medium or an error in the recorded data. This sense key may also be returned if the device server is unable to distinguish between a flaw in the medium and a specific hardware failure." + Environment.NewLine + Environment.NewLine + "Details: " + GetMessage(senseData.SenseKey, senseData.AdditionalSenseCodeAndQualifier), ex);
						break;
					case SenseKey.HardwareError:
						ex = new IOException("The device server detected a non-recoverable hardware failure (e.g., controller failure, device failure, or parity error) while performing the command or during a self test." + Environment.NewLine + Environment.NewLine + "Details: " + GetMessage(senseData.SenseKey, senseData.AdditionalSenseCodeAndQualifier), ex);
						break;
					case SenseKey.IllegalRequest:
						ex = new InvalidOperationException("An illegal operation was requested." + Environment.NewLine + Environment.NewLine + "Details: " + GetMessage(senseData.SenseKey, senseData.AdditionalSenseCodeAndQualifier), ex);
						break;
					case SenseKey.UnitAttention:
						ex = new IOException("A unit attention condition has been established (e.g., the removable medium may have been changed, a logical unit reset occurred)." + Environment.NewLine + Environment.NewLine + "Details: " + GetMessage(senseData.SenseKey, senseData.AdditionalSenseCodeAndQualifier), ex);
						break;
					case SenseKey.DataProtect:
						ex = new UnauthorizedAccessException("That a command that reads or writes the medium was attempted on a block that is protected. The read or write operation is not performed." + Environment.NewLine + Environment.NewLine + "Details: " + GetMessage(senseData.SenseKey, senseData.AdditionalSenseCodeAndQualifier), ex);
						break;
					case SenseKey.BlankCheck:
						ex = new IOException("A write-once device or a sequential-access device encountered blank medium or format-defined end-of-data indication while reading or that a write-once device encountered a non-blank medium while writing." + Environment.NewLine + Environment.NewLine + "Details: " + GetMessage(senseData.SenseKey, senseData.AdditionalSenseCodeAndQualifier), ex);
						break;
					case SenseKey.CopyAborted:
						ex = new IOException("An EXTENDED COPY command was aborted due to an error condition on the source device, the destination device, or both." + Environment.NewLine + Environment.NewLine + "Details: " + GetMessage(senseData.SenseKey, senseData.AdditionalSenseCodeAndQualifier), ex);
						break;
					case SenseKey.AbortedCommand:
						ex = new OperationCanceledException("The device server aborted the command. The application client may be able to recover by trying the command again." + Environment.NewLine + Environment.NewLine + "Details: " + GetMessage(senseData.SenseKey, senseData.AdditionalSenseCodeAndQualifier), ex);
						break;
					case SenseKey.VolumeOverflow:
						ex = new EndOfStreamException("A buffered SCSI device has reached the end-of-partition and data may remain in the buffer that has not been written to the medium. One or more RECOVER BUFFERED DATA command(s) may be issued to read the unwritten data from the buffer." + Environment.NewLine + Environment.NewLine + "Details: " + GetMessage(senseData.SenseKey, senseData.AdditionalSenseCodeAndQualifier), ex);
						break;
					case SenseKey.Miscompare:
						ex = new InvalidDataException("The source data did not match the data read from the medium." + Environment.NewLine + Environment.NewLine + "Details: " + GetMessage(senseData.SenseKey, senseData.AdditionalSenseCodeAndQualifier), ex);
						break;
				}
			}
			return ex;
