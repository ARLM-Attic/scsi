...
namespace Scsi
{
	...
	{
		ScsiStatus ExecuteCommand(BufferWithSize cdb, DataTransferDirection direction, byte pathId, byte targetId, byte logicalUnitNumber, BufferWithSize data, uint timeout, bool autoSense, out SenseData senseData);
		...
		TFeature GetCdromConfiguration<TFeature>() where TFeature : Multimedia.MultimediaFeature;
		Multimedia.FeatureCollection GetCdromConfiguration(Multimedia.FeatureCode startingFeature, Multimedia.FeatureRequestType requestType);
		Multimedia.MultimediaProfile CurrentCdromProfile { get; }
		...
		KeyValuePair<byte, KeyValuePair<ScsiDeviceInfo, StandardInquiryData>[]>[] GetScsiInquiryData(); //Deprecated, but who cares?!
	...
	{
		public ScsiDeviceInfo(byte pathId, byte targetId, byte lun, bool claimed) { this.PathId = pathId; this.TargetId = targetId; this.Lun = lun; this.DeviceClaimed = claimed; }
		...
		public readonly bool DeviceClaimed;
	}
}