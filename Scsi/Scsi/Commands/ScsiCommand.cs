...
namespace Scsi
{
	...
	{
		protected ScsiCommand(ScsiCommandCode operationCode) : base() { this.OpCode = operationCode; }

		[DebuggerBrowsable(DebuggerBrowsableState.Never)]
		internal ScsiCommandCode _OperationCode;  //Internal so we can pin it
		public ScsiCommandCode OpCode { get { return this._OperationCode; } private set { this._OperationCode = value; } }
		...
		{
			var result = TryCreateInstance(operationCode);
			if (result == null) { throw new ArgumentOutOfRangeException("operationCode", operationCode, "Unsupported OpCode."); }
			return result;
		}
		...
		public static ScsiCommand TryCreateInstance(ScsiCommandCode operationCode)
		{
			...
			switch (operationCode)
			{
				...
				case ScsiCommandCode.LoadUnloadMedium:
					...
					break;
				case ScsiCommandCode.ModeSelect06:
					result = new ModeSelect06Command();
					break;
				case ScsiCommandCode.ModeSelect10:
					result = new ModeSelect10Command();
					break;
				case ScsiCommandCode.ModeSense06:
					result = new ModeSense06Command();
					break;
				...
				case ScsiCommandCode.Read06:
					...
					break;
				case ScsiCommandCode.Release06:
					result = new Release06Command();
					break;
				case ScsiCommandCode.Release10:
					result = new Release10Command();
					break;
				case ScsiCommandCode.ReportKey:
					result = new ReportKeyCommand();
					break;
				case ScsiCommandCode.ReportLuns:
					result = new ReportLunsCommand();
					break;
				case ScsiCommandCode.RequestSense:
					result = new RequestSenseCommand();
					break;
				case ScsiCommandCode.Reserve06:
					result = new Reserve06Command();
					break;
				case ScsiCommandCode.Reserve10:
					result = new Reserve10Command();
					break;
		...
		public static ScsiCommand FromBuffer(IntPtr pBuffer, int bufferLength)
		{
			...
			var instance = CreateInstance(buffer.Read<ScsiCommandCode>());
		...
		{
			var buffer = new BufferWithSize(pBuffer, bufferLength);
			var instance = TryCreateInstance(buffer.Read<ScsiCommandCode>());
			if (instance != null) { Marshaler.PtrToStructure(buffer, ref instance); }
			return instance;
		}

		public static ScsiCommandCode ReadOperationCode(IntPtr cdb, int cdbLength) { return new BufferWithSize(cdb, cdbLength).Read<ScsiCommandCode>(); }
	}
	...
	public struct CommandControl
	{
		...
		private byte byte0;
		[DebuggerBrowsable(DebuggerBrowsableState.Never)]
		public bool Link { get { return Bits.GetBit(this.byte0, 0); } set { this.byte0 = Bits.SetBit(this.byte0, 0, value); } }
		[DebuggerBrowsable(DebuggerBrowsableState.Never)]
		public bool NormalACA { get { return Bits.GetBit(this.byte0, 2); } set { this.byte0 = Bits.SetBit(this.byte0, 2, value); } }
		public override string ToString() { return string.Format("Lined = {0}, NACA = {1}", this.Link, this.NormalACA); }
	}
}